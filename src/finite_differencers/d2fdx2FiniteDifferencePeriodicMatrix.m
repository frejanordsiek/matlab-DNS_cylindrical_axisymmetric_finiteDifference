function coeffMatrix = d2fdx2FiniteDifferencePeriodicMatrix(f,h,dim)
%D2FDX2FINITEDIFFERENCEPERIODICMATRIX finds the coeff matrix for the second finite difference on a periodic uniform grid.
% COEFFMATRIX = D2FDX2FINITEDIFFERENCEPERIODICMATRIX(F, H, DIM)
% Creates the coefficient matrix, COEFFMATRIX which is sparse, to take the
% second centered finite difference of F on a periodic uniform grid along
...
% we simplify things can calculate the index to which (1:23) must be added
...
% are simply given by the formula. The two boundaries and the interior are then
% handled separately.
...
switch dim
    case 1
        for ii=1:siz(1)
            for jj=1:siz(2)
                for kk=1:siz(3)
                    baseIndex = 3*((kk-1) + (jj-1)*siz(3) + (ii-1)*siz(3)*siz(2));
                    rows(baseIndex + (1:3)) = sub2ind(siz,ii,jj,kk)+[0;0;0];
                    switch ii
                        case 1
                            cols(baseIndex + (1:3)) = sub2ind(siz,[2;1;siz(1)],jj+[0;0;0],kk+[0;0;0]);
                        case siz(1)
                            cols(baseIndex + (1:3)) = sub2ind(siz,[1;siz(1);siz(1)-1],jj+[0;0;0],kk+[0;0;0]);
                        otherwise
                            cols(baseIndex + (1:3)) = sub2ind(siz,ii+[1;0;-1],jj+[0;0;0],kk+[0;0;0]);
                    end
                    coeffs(baseIndex + (1:3)) = [1; -2; 1]/(h^2);
                end
            end
        end
    case 2
        for ii=1:siz(1)
            for jj=1:siz(2)
                for kk=1:siz(3)
                    ...
                    switch jj
                        case 1
                            cols(baseIndex + (1:3)) = sub2ind(siz,ii+[0;0;0],[2;1;siz(2)],kk+[0;0;0]);
                        case siz(2)
                            cols(baseIndex + (1:3)) = sub2ind(siz,ii+[0;0;0],[1;siz(2);siz(2)-1],kk+[0;0;0]);
                        otherwise
        ...
        for ii=1:siz(1)
            for jj=1:siz(2)
                for kk=1:siz(3)
                    ...
                    switch kk
                        case 1
                            cols(baseIndex + (1:3)) = sub2ind(siz,ii+[0;0;0],jj+[0;0;0],[2;1;siz(3)]);
                        case siz(3)
                            cols(baseIndex + (1:3)) = sub2ind(siz,ii+[0;0;0],jj+[0;0;0],[1;siz(3);siz(3)-1]);
                        otherwise
