function DfDx = dfdxFiniteDifferencePeriodic(f,h,dim)
%DFDXFINITEDIFFERENCEPERIODIC does first finite difference on a periodic uniform grid.
% DFDX = DFDXFINITEDIFFERENCEPERIODIC(F, H, DIM)
% Does the first centered finite difference on a periodic uniform grid
...
switch dim
    case 1
        DfDx = dfdxFiniteDifference(cat(1,f(end,:,:),f,f(1,:,:)),h,dim);
    case 2
        DfDx = dfdxFiniteDifference(cat(2,f(:,end,:),f,f(:,1,:)),h,dim);
    case 3
        DfDx = dfdxFiniteDifference(cat(3,f(:,:,end),f,f(:,:,1)),h,dim);
    otherwise
