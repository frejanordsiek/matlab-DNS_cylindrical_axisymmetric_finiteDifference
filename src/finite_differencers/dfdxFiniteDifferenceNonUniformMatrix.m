function coeffMatrix = dfdxFiniteDifferenceNonUniformMatrix(f,x,dim)
%DFDXFINITEDIFFERENCENONUNIFORMMATRIX finds coeff matrix for the first finite difference on a non-uniform grid.
% COEFFMATRIX = DFDXFINITEDIFFERENCENONUNIFORMMATRIX(F, X, DIM)
% Creates the coefficient matrix, COEFFMATRIX which is sparse, to take the
% first centered finite difference of F on a non-uniform grid along
...
switch dim
    ...
    case 2
        for ii=1:siz(1)
            for jj=2:(siz(2)-1)
                for kk=1:siz(3)
                    baseIndex = 2*((kk-1) + (jj-2)*siz(3) + (ii-1)*siz(3)*(siz(2)-2));
                    rows(baseIndex + (1:2)) = sub2ind(siz,ii,jj,kk)+[0;0];
                    cols(baseIndex + (1:2)) = sub2ind(siz,ii+[0;0],jj+[1;-1],kk+[0;0]);
                    coeffs(baseIndex + (1:2)) = [1;-1]/(x(jj+1)-x(jj-1));
                end
            end
        end
    case 3
        for ii=1:siz(1)
            for jj=1:siz(2)
                for kk=2:(siz(3)-1)
                    baseIndex = 2*((kk-2) + (jj-1)*(siz(3)-2) + (ii-1)*(siz(3)-2)*siz(2));
                    rows(baseIndex + (1:2)) = sub2ind(siz,ii,jj,kk)+[0;0];
                    cols(baseIndex + (1:2)) = sub2ind(siz,ii+[0;0],jj+[0;0],kk+[1;-1]);
                    coeffs(baseIndex + (1:2)) = [1;-1]/(x(kk+1)-x(kk-1));
                end
            end
        end
    otherwise
